...
from api.models import Role, Task, Knowledge, DataFlow, Organization
...
class OpenClawAdapter:
    ...
    def _get_agent_path(self, org_id: str, role_name: str) -> str:
        agent_dir = os.path.join(self.openclaw_path, org_id, role_name.lower().replace(" ", "_"))
        os.makedirs(agent_dir, exist_ok=True)
        os.makedirs(os.path.join(agent_dir, ".openclaw"), exist_ok=True)
        os.makedirs(os.path.join(agent_dir, "skills"), exist_ok=True)
        os.makedirs(os.path.join(agent_dir, "memory"), exist_ok=True)
        return agent_dir

    def deploy_role(self, org_id: str, role: Role, knowledge_base: List[Knowledge] = None) -> str:
        ...
        agents_content = self._generate_agents_md(role, knowledge_base)
...
"""
    ...
    def _generate_agents_md(self, role: Role, knowledge_base: List[Knowledge] = None) -> str:
        knowledge_section = ""
        if knowledge_base:
            kb_list = "\n".join([f"- {kb.title}: {kb.content[:50]}..." for kb in knowledge_base[:5]])
            knowledge_section = f"\n## 📚 Knowledge Base\n\n{kb_list}\n"
...
"""
    ...
    def undeploy_role(self, org_id: str, role_name: str) -> bool:
        agent_path = self._get_agent_path(org_id, role_name)
    ...
    def undeploy_organization(self, org_id: str) -> bool:
        org_path = os.path.join(self.openclaw_path, org_id)
        if os.path.exists(org_path):
            shutil.rmtree(org_path)
            return True
        return False
